...
// We include Core Foundation here for CFString handling.
...
bool DeviceInfo::isValid() const {
   // A device is valid if it has a non-zero device ID
...
std::vector<DeviceInfo> DeviceManager::enumerateDevices() const {
   // We enumerate all audio devices on the system by
   ...
   for (AudioDeviceID deviceID : deviceIDs) {
      ...
      // We only include devices that have a valid name
   ...
   // Store the enumerated devices in the member
...
DeviceManager::findDeviceByName(const std::string& deviceName) const {
   // We search the enumerated devices for one whose
   ...
   // We convert the device name to lowercase for
...
AudioFormat DeviceManager::getDeviceFormat(const DeviceInfo& device) const {
   // We query Core Audio for the stream configuration
...
std::string DeviceManager::getDeviceName(AudioDeviceID deviceID) const {
   ...
   AudioObjectPropertyAddress address = {
      ...
      kAudioObjectPropertyElementMain // Use the non-deprecated
                                      // name.
   };
   ...
   // We call AudioObjectGetPropertyData to get the
   ...
   // We convert the CFStringRef to a std::string.
   ...
   // We must release the CFStringRef to avoid a leak.
...
AudioFormat DeviceManager::getStreamConfig(AudioDeviceID deviceID) const {
   ...
   // We first get the size of the buffer list,
   ...
   if (status != noErr || bufferSize == 0) {
      // If we can't get the buffer size, fall back
      // to kAudioDevicePropertyStreamFormat.
      return getStreamFormat(deviceID);
   }

   // We allocate the buffer list and read the data.
   ...
   if (status != noErr || bufferList->mNumberBuffers == 0) {
      free(bufferList);
      // If we can't get the buffer list, fall back
      // to kAudioDevicePropertyStreamFormat.
      return getStreamFormat(deviceID);
   }

   // We extract the AudioStreamBasicDescription from
   ...
   // MData is void*, so we cast it to the correct type.
   // But first, we check that mData is not nullptr.
   if (bufferList->mBuffers[0].mData == nullptr) {
      free(bufferList);
      // MData is nullptr — device is not active.
   ...
   // We convert the AudioStreamBasicDescription to
...
AudioFormat DeviceManager::getStreamFormat(AudioDeviceID deviceID) const {
   // We query Core Audio for the stream format of the
...
std::vector<AudioDeviceID> DeviceManager::enumerateDeviceIDs() const {
   // We enumerate all audio device IDs on the system
   // by querying Core Audio for the list of devices. This is the
   // standard way to enumerate devices.

   // KAudioObjectPropertyList was removed in newer macOS.
   ...
   // We first get the size of the device list,
   ...
   // We allocate the device list and read the data.
   ...
   // We copy the device IDs into a vector.
   ...
   // We must free the deviceIDs array to avoid a leak.
   free(deviceIDs);
