// Raw probe: per-frame RMS + aubio internal thresholds + onset events.
...
int main(int argc, char** argv) {
   ...
   float silenceDb = argc > 5 ? std::atof(argv[5]) : -40.0f;
   SF_INFO info{};
   SNDFILE* f = sf_open(path, SFM_READ, &info);
   if (!f) return 1;
   std::printf("sr=%d ch=%d frames=%ld thr=%.3f silenceDb=%.1f\n",
   ...
   long lastOnset = -1000;
   for (long frame = 0; frame <= info.frames; frame += hop) {
      long n = sf_seek(f, frame, SEEK_SET) ? 0
                    : sf_readf_float(f, raw.data(), window);
      if (n <= 0) break;
      for (long i = 0; i < n; ++i)
      ...
      float thr = aubio_onset_get_threshold(o);
      bool fire = thdes != 0.0f;
      if (fire) lastOnset = frame;
      if (frame % (hop * 2) == 0)
         std::printf("f=%6ld t=%.3f rms=%.4f desc=%9.3f thr=%6.3f thDesc=%6.3f lastOnset=%ld\n",
                     frame, frame / (double)info.samplerate, rms, des, thr,
                     thdes, lastOnset);
   }
   sf_close(f);
