// Full-file onset probe: sequential hop reading, counts all onsets.
...
int main(int argc, char** argv) {
   ...
   while (true) {
      ...
      if (first) {
         // aubio reports t=0.0 before any detection; skip that state.
      ...
      if (t > lastT) {
         count++;
         lastT = t;
         if (count <= 30) std::printf("onset %2ld at t=%.3f\n", count, t);
      }
   }
   std::printf("TOTAL onsets: %ld\n", count);
   sf_close(f);
