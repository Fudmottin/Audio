// Onset probe: full-file, configurable minioi, counts via get_last deltas.
// Usage: probe8 <file> [window] [hop] [threshold] [minIoI_sec]
...
int main(int argc, char** argv) {
   ...
   while (true) {
      ...
      if (t > lastT) {
         count++;
         lastT = t;
         if (count <= 30) std::printf("onset %2ld at t=%.4f (idx=%ld)\n", count, t, idx);
      }
      idx++;
   }
   std::printf("TOTAL onsets: %ld  (minioi=%.3f s)\n", count,
               minIoI >= 0 ? minIoI : aubio_onset_get_minioi_s(o));
   sf_close(f);
