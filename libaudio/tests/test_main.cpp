...
// ============================================================================
static void writeTestTone(const char* path,
                          double frequency,
                          double sampleRate,
                          uint32_t numSamples)
{
    ...
    std::vector<float> samples = generateSineWave(
        frequency, sampleRate, numSamples);

    int written = sf_writef_float(sf, samples.data(),
...
static void test_hir()
{
    ...
    Note defaultNote;
    ASSERT(defaultNote.pitch == 60,
           "HIR: default Note.pitch == 60 (middle C)");
    ASSERT(defaultNote.velocity == 100,
           "HIR: default Note.velocity == 100");
    ASSERT(defaultNote.channel == 0,
           "HIR: default Note.channel == 0");
    ASSERT(defaultNote.sustain == false,
           "HIR: default Note.sustain == false");
    ASSERT(defaultNote.startTime == 0.0,
           "HIR: default Note.startTime == 0.0");
    ASSERT(defaultNote.endTime == 0.0,
           "HIR: default Note.endTime == 0.0");

    // 1b. Default-constructed ControlEvent has correct defaults.
    ControlEvent defaultCtrl;
    ASSERT(defaultCtrl.time == 0.0,
           "HIR: default ControlEvent.time == 0.0");
    ASSERT(defaultCtrl.controller == 0,
           "HIR: default ControlEvent.controller == 0");
    ASSERT(defaultCtrl.value == 0,
           "HIR: default ControlEvent.value == 0");

    // 1c. Default-constructed Score has correct defaults.
    Score defaultScore;
    ASSERT(defaultScore.tempo == 120.0,
           "HIR: default Score.tempo == 120.0");
    ASSERT(defaultScore.notes.empty(),
           "HIR: default Score.notes is empty");
    ASSERT(defaultScore.controls.empty(),
           "HIR: default Score.controls is empty");
    ASSERT(defaultScore.title.empty(),
           "HIR: default Score.title is empty");
    ...
    ASSERT(explicitNote.startTime == 1.5,
           "HIR: explicit Note.startTime == 1.5");
    ASSERT(explicitNote.endTime == 2.0,
           "HIR: explicit Note.endTime == 2.0");
    ASSERT(explicitNote.pitch == 64,
           "HIR: explicit Note.pitch == 64 (E4)");
    ASSERT(explicitNote.velocity == 120,
           "HIR: explicit Note.velocity == 120");
    ASSERT(explicitNote.channel == 1,
           "HIR: explicit Note.channel == 1");
    ASSERT(explicitNote.sustain == true,
           "HIR: explicit Note.sustain == true");
    ...
    ASSERT(sustainEvent.time == 0.0,
           "HIR: explicit ControlEvent.time == 0.0");
    ...
    Score composedScore;
    Note n1;
    n1.startTime = 0.0; n1.endTime = 0.5;
    n1.pitch = 60; n1.velocity = 100; n1.channel = 0; n1.sustain = false;
    composedScore.notes.push_back(n1);

    Note n2;
    n2.startTime = 0.5; n2.endTime = 1.0;
    n2.pitch = 64; n2.velocity = 100; n2.channel = 0; n2.sustain = false;
    ...
    ASSERT(composedScore.tempo == 120.0,
           "HIR: composed Score.tempo == 120.0");
    ...
    Note n3;
    n3.startTime = 1.5; n3.endTime = 2.0;
    n3.pitch = 67; n3.velocity = 80; n3.channel = 0; n3.sustain = false;
    ...
    ctrlOff.time = 2.0; ctrlOff.controller = 64; ctrlOff.value = 0;
    ...
    ASSERT(noteIterCount == 3,
           "HIR: can iterate over Score.notes (count == 3)");
...
static void test_fft()
{
    ...
    FFT fft(2048);
    ASSERT(fft.windowSize() == 2048,
           "FFT: windowSize() returns 2048");
    ASSERT(fft.numBins() == 1025,
           "FFT: numBins() returns 1025 (2048/2+1)");
    ...
    std::vector<float> sineWave = generateSineWave(
        frequency, sampleRate, windowSize);
    ...
    ASSERT(binDiff < 1.5,
           "FFT: peak bin is near expected 440 Hz bin (got bin " +
...
static void test_pitch()
{
    ...
    ASSERT(detector.bufSize() == 2048,
           "Pitch: bufSize() returns 2048");
    ...
    ASSERT(silenceConf == 0.0f,
           "Pitch: silence returns confidence 0.0");
    ...
    ASSERT(storedConf == 0.0f,
           "Pitch: confidence() returns 0.0 for silence");
    ...
    ASSERT(thresholdDet.confidenceThreshold() == 0.75f,
...
static void test_scoreBuilder()
{
    ...
    ASSERT(builder.noteCount() == 0,
           "ScoreBuilder: default noteCount() == 0");
    ASSERT(builder.controlEventCount() == 0,
           "ScoreBuilder: default controlEventCount() == 0");

    // 4b. Add 3 notes, verify noteCount().
    Note note1, note2, note3;
    note1.startTime = 0.0; note1.endTime = 0.5;
    note1.pitch = 60; note1.velocity = 100; note1.channel = 0; note1.sustain = false;
    builder.addNote(note1);

    note2.startTime = 0.5; note2.endTime = 1.0;
    note2.pitch = 64; note2.velocity = 90; note2.channel = 0; note2.sustain = false;
    builder.addNote(note2);

    note3.startTime = 1.0; note3.endTime = 1.5;
    note3.pitch = 67; note3.velocity = 80; note3.channel = 0; note3.sustain = false;
    ...
    ctrlOn.time = 0.0; ctrlOn.controller = 64; ctrlOn.value = 127;  // Sustain ON
    builder.addControlEvent(ctrlOn);

    ctrlOff.time = 1.5; ctrlOff.controller = 64; ctrlOff.value = 0;  // Sustain OFF
...
static void test_midiWriter()
{
    ...
    Note m1, m2;
    m1.startTime = 0.0; m1.endTime = 0.5;
    m1.pitch = 60; m1.velocity = 100; m1.channel = 0; m1.sustain = false;
    midiScore.notes.push_back(m1);  // C4

    m2.startTime = 0.5; m2.endTime = 1.0;
...
static void test_audioFileReader()
{
    ...
    ASSERT(!reader.eof(),
           "Audio: after seek to 24000, eof() is false");
    ...
    ASSERT(reader.eof(),
           "Audio: after reading past EOF, eof() returns true");
}
